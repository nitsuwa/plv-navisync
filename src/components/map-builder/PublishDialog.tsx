import { X, Building2, Layers, DoorOpen, AlertTriangle, CheckCircle2, Globe } from "lucide-react";
...
export function PublishDialog({ campus, onClose, onPublish }: PublishDialogProps) {
  ...
  return (
    ...
    >
      <div
        className="bg-card border border-border rounded-2xl shadow-2xl w-full max-w-lg animate-scale-in overflow-hidden flex flex-col"
      ...
      >
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {[
                ...
                <div
                  key={stat.label}
                  className="flex flex-col items-center gap-1.5 py-3 rounded-xl border border-border bg-muted/20"
                >
            ...
            <div>
              ...
              <div className="space-y-1.5">
                {warnings.map((w, i) => (
                  <div
                    key={i}
                    className="flex items-start gap-2 px-3 py-2 rounded-xl border border-amber-200 dark:border-amber-800/30 bg-amber-50 dark:bg-amber-900/10"
                  >
                    <AlertTriangle className="h-3.5 w-3.5 text-amber-500 shrink-0 mt-0.5" />
                    <span className="text-xs text-amber-800 dark:text-amber-300" style={{ fontFamily: "var(--font-body)" }}>
                      {w}
                    </span>
                  </div>
                ))}
          ...
          {!hasWarnings && (
            <div className="flex items-center gap-2 px-4 py-3 rounded-xl border border-green-200 dark:border-green-800/30 bg-green-50 dark:bg-green-900/10">
              ...
              <span className="text-xs font-semibold text-green-700 dark:text-green-300">
                All checks passed. This campus is ready to publish.
              </span>
            </div>
          )}
        </div>
        ...
        <div className="flex gap-2 px-6 pb-5 pt-3 border-t border-border shrink-0">
          ...
          <button
            onClick={onPublish}
            className="flex-1 h-10 rounded-xl bg-primary text-primary-foreground text-sm font-extrabold hover:bg-primary/90 transition-colors flex items-center justify-center gap-2 shadow-sm"
          >
