...
function fixture(): FloorPlan {
  return {
    ...
    doors: [{ id: "door-1", x: 40, y: 100, width: 12, wallId: "wall-room", offset: 0.3, direction: "left", color: "#111827", buildingEntranceId: "entrance-1" }],
    windows: [{ id: "window-1", x: 80, y: 30, width: 30, height: 5, wallId: "wall-room", offset: 0.2, color: "#38bdf8" }],
    furniture: [{ id: "furniture-1", type: "computer-lab-table-4", name: "Computer Lab Table 4", category: "electronics", x: 60, y: 70, width: 80, height: 30, rotation: 0, color: "#475569" }],
...
describe("custom template sanitizers", () => {
  ...
  it("uses a positive allow-list for a Floor and strips all navigation-sensitive collections", () => {
    ...
    expect(JSON.stringify(result)).not.toMatch(/door|stairs|elevator|ramp|pathway|navNode|navEdge|entrance|transition|groundDischarge|exteriorEmergencyStair/i);
