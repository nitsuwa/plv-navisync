...
// Vite variables AND the corresponding disposable demo credentials are
// configured. VITE_* values are public in the built browser application, so
// these variables must never contain credentials for a real account.
...
// included when its own disposable email and password are configured (admin
// and student are independent; both still require demo mode enabled).
...
export function AdminLoginPage() {
  ...
  // Selecting a disposable demo account fills its configured credentials and
  // clears any stale error. It never signs the user in — Sign In still runs
  // through supabase.auth.signInWithPassword() with the normal checks.
              ...
              <div ref={demoRef} className="mb-6">
                ...
                >
                  ...
                  <span className="flex-1 text-left truncate">
                    {selectedDemoId
                      ? `${DEMO_ACCOUNTS.find(a => a.id === selectedDemoId)?.label ?? "Demo account"} credentials applied`
                ...
                <p className="text-[11px] text-muted-foreground mt-2 leading-relaxed">
                  Selecting an account fills its disposable demo credentials.
                  Press{" "}
